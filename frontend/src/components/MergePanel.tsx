...
function ConflictRow({
  ...
  row: MergeRow;
  useIncoming: boolean;
  toggle: (key: string, v: boolean) => void;
}) {
    ...
    >
      <Group justify="space-between">
        ...
        <Badge size="xs" color="yellow" variant="light">
          {row.diffs.length} field(s) differ
        </Badge>
      </Group>
      {row.diffs.map((f) => (
        <Text key={f} size="xs" c="dimmed">
          <b>{f}:</b> {String(row.existing?.[f] ?? "—")}{" "}
          <span style={{ color: "var(--mantine-color-gray-5)" }}>→</span>{" "}
          {String(row.incoming[f] ?? "—")}
        </Text>
      ))}
      <Radio.Group
      ...
      >
        <Group gap="lg" mt={2}>
          <Radio
            value="current"
            label="Keep current"
            data-testid="merge-keep-current"
          />
          <Radio
            value="incoming"
            label="Use incoming"
            data-testid="merge-use-incoming"
          />
        </Group>
      </Radio.Group>
    </Stack>
...
export function MergePanel({ token }: { token: string }) {
  ...
  const browse = async () => {
    if (window.leosDesktop) {
      const selected = await window.leosDesktop.chooseImportFile("database");
  ...
  const previewMut = useMutation({
    mutationFn: () =>
      postJSON(token, "/import/merge/preview", { path }) as Promise<Preview>,
    onSuccess: (data) => {
      setPreview(data);
  ...
  const buildOps = (rows: MergeRow[], entity: string) =>
    rows.flatMap((r, i) => {
      if (r.action === "new")
        return importNew ? [{ action: "insert", data: r.incoming }] : [];
    ...
    rows
      ...
      .map(({ r, i }) => (
        <ConflictRow
          ...
          idx={i}
          row={r}
          useIncoming={useIncoming.has(`${entity}:${i}`)}
          toggle={toggle}
        />
      ));
  ...
  return (
    <Stack gap="md">
      <Text size="sm" c="dimmed">
        Merge another school file (a <code>.leosdb</code> someone edited
        separately) into this one. LEOS matches students by{" "}
        <b>name + date of birth</b> (or Admission No / email when present) and
        staff by name/email, then shows new records, identical rows, and
        conflicts to reconcile before anything is written.
      </Text>
      ...
      <Group align="flex-end" gap="sm" wrap="nowrap">
        <TextInput
          ...
          rightSection={
            window.leosDesktop ? (
              <Button size="compact-xs" variant="light" onClick={browse}>
                Browse…
              </Button>
            ) : undefined
          }
      ...
      {done && (
        ...
        >
          {done.inserted} record(s) imported, {done.updated} reconciled. The
      ...
      {preview && (
        <Stack gap="sm" data-testid="merge-preview">
          ...
          {preview.summary.students.conflict + preview.summary.staff.conflict >
            0 && (
            <>
              <Divider label="Conflicts to reconcile" labelPosition="left" />
              <Text size="xs" c="dimmed">
                Default is to keep your current data. Switch to “Use incoming”
                to take the other file’s values.
              </Text>
      ...
      {!preview && !done && (
        <Group gap="xs" c="dimmed">
          ...
          <Text size="xs" c="dimmed">
            Pick a file and Preview to see what would change. Nothing is written
            until you Apply.
          </Text>
        </Group>
      )}

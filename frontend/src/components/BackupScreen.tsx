...
async function apiConfig(token: string): Promise<{ config: BackupConfig }> {
  return fetch(`${BASE}/backup/config`, { headers: authed(token) }).then((r) =>
    r.json(),
  );
}

async function apiList(token: string): Promise<{ backups: BackupEntry[] }> {
  return fetch(`${BASE}/backup/list`, { headers: authed(token) }).then((r) =>
    r.json(),
  );
}
...
export function BackupScreen() {
  ...
  const [restoreMasterKey, setRestoreMasterKey] = useState("");
  ...
  const runMut = useMutation({
    ...
    onSuccess: (res) => {
      ...
      setLastResult(
        res.ok
          ? `Saved: ${res.filename} (${fmtBytes(res.size_bytes)})`
          : `Error: ${res.error}`,
      );
    },
  });
  ...
  const restoreMut = useMutation({
    mutationFn: () =>
      postJSON(token, "/backup/restore", {
        path: restorePath,
        master_key: restoreMasterKey,
      }),
    onSuccess: (res) => {
      setConfirmRestore(false);
      setLastResult(
        res.ok ? `Restored from ${res.restored_from}` : `Error: ${res.error}`,
      );
      if (res.requires_relogin) signOut();
      else qc.invalidateQueries({ queryKey: ["backup-config"] });
    },
  });
      ...
      <Stack gap="lg">
            ...
            <Stack gap="sm">
                  ...
                  rightSection={
                    window.leosDesktop ? (
                      <Button
                        ...
                        variant="light"
                        onClick={async () => {
                          const d = await window.leosDesktop?.chooseFolder();
                          if (d) setDestDir(d);
                        }}
                      >
                        Browse…
              ...
              {config?.last_backup_at && (
                <Text size="xs" c="dimmed">
                  Last backup: {config.last_backup_at}
                </Text>
              )}
            </Stack>
          )}
        ...
        <Card>
          ...
          {lastResult && (
            <Group gap="xs">
              ...
              <Text
                size="sm"
                c={lastResult.startsWith("Error") ? "red" : "green"}
              >
                {lastResult}
              </Text>
            </Group>
        ...
        <Card style={{ borderTop: "3px solid var(--mantine-color-peach-5)" }}>
          ...
          <Text size="xs" c="dimmed" mb="sm">
            This will REPLACE the current database with the selected backup. All
            data created after that backup will be lost.
          </Text>
          <Stack gap="sm">
            <TextInput
              ...
              value={restorePath}
              onChange={(e) => {
                setRestorePath(e.currentTarget.value);
                setConfirmRestore(false);
              }}
              ...
              rightSection={
                window.leosDesktop ? (
                  <Button
                    ...
                    variant="light"
                    onClick={async () => {
                      const f = await window.leosDesktop?.chooseSchoolFile();
                      if (f) {
                        setRestorePath(f);
                        setConfirmRestore(false);
                      }
            ...
            <PasswordInput
              label="Master key"
              description="Required to verify that this backup belongs to your school."
              value={restoreMasterKey}
              onChange={(event) => {
                setRestoreMasterKey(event.currentTarget.value);
                setConfirmRestore(false);
              }}
            />
            {restorePath && restoreMasterKey && !confirmRestore && (
              <Button
                variant="outline"
                color="red"
